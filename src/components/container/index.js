...
const UIContainer = UIBase.extend(module, {
	doInit: function (done) {
		UIContainer.super(this, () => {
	...
	setPaddings: function (top, right, bottom, left) {
		top = Utils.isBlank(top) ? 0 : top;
		right = Utils.isBlank(right) ? top : right;
		bottom = Utils.isBlank(bottom) ? top : bottom;
		left = Utils.isBlank(left) ? right : left;
